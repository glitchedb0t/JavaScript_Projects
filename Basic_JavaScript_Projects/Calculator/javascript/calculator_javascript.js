...
function Input_Digit(digit) {
    ...
    // This checks if the Wait_Second_Operand is true and sets Display_Value to the
...
function Handle_Operator(Next_Operator) {
    ...
    // When an operator key is pressed we convert the current number displayed on
    //the screen to a number and then store the result in Calculator.First_Operand
    //if it doesn't already exist.
    const Value_of_Input = parseFloat(Display_Value);
    // Checks if an operator already exists and if Wait_Second_Operand is true, then
    ...
    if (First_Operand == null) {
        ...
        //Perform_Calculation object and the function that matches the operator is
        //executed.
...
function Update_Display() {
    // Makes use of the calculator-screen class to target the input tag in the 
    //HTML document.
